  ...
  if (!services || services.length === 0) {
    container.innerHTML = `
      <div class="col-span-1 md:col-span-3 card-terminal border border-violet-500/30 p-8 rounded-3xl text-center space-y-3">
        ...
        <p class="text-gray-500 text-xs mono-font">يرجى التأكد من إضافة وتفعيل الخدمات داخل حساب AIVerse أو الشيت.</p>
...
function checkUrlForTrackingToken() {
  ...
  if (token) {
    ...
    fetchAndRenderOrder(token);
  }
}

document.addEventListener("DOMContentLoaded", () => {
  loadProducts();
  checkUrlForTrackingToken();
});
